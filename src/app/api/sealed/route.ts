...
export async function GET() {
  ...
  const url = `https://api.ebay.com/buy/browse/v1/item_summary/search?q=${encodeURIComponent(
    query
  )}&filter=itemLocationCountry:GB,buyingOptions:{FIXED_PRICE}&limit=24&sort=price`;
  ...
  try {
    ...
    const affiliateItems = items.map((item) => {
      const rawPrice = parseFloat(item.price.value);
      const currency = item.price.currency;
      const price =
        currency === 'USD'
          ? Math.round(rawPrice * 0.79 * 100) / 100 // static USD→GBP fallback
          : rawPrice;

      return {
        title: item.title,
        price,
        currency: 'GBP', // always display as GBP after conversion
